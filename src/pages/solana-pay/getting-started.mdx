...
Clone the repository, checkout the correct branch, and install dependencies:
 ```bash
 ...
 This project uses [dotenv](https://nextjs.org/docs/basic-features/environment-variables) style configuration files to manage environment variables. You can see all the variables you will need to define in the `.env.example` file in the root of the project. Make a copy of this file called `.env` and place it in the root of the project. You will be defining the values for these variables throughout the rest of this section. **Note:** the `.env` file should not be committed to git and is ignore by default.
...
Update the `FAUCET_SECRET_KEY_PATH` value in the `.env` file in the root of the project with the absolute path to the keypair you just created. For example, my path looks like:
 ...
 environment variable in `.env` with the connection string for your database. For example, my `.env` file looks like:
...
[section 4](/solana-pay/real-time-updates)) and check to see that the UI has updated with the correct number of tokens. It should look 
something like this:
...
Our faucet is now setup! Spend some time familiarizing yourself with the application as it currently exists. 
