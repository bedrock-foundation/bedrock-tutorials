...
import taskService from './services/task/task.service'; // Add this line

export const appRouter = router({
  faucet: faucetService.router,
  scan: scanService.router,
  task: taskService.router, // Add this line
});
...
taskService.setAppCaller(appCaller); // Add this line
...
Using an task queue with state external to your application is a common pattern for durable tracking. In this tutorial, we used Redis and BullMQ. Several alternative approaches. While we will not explore these options further
in this series, they are worth mentioning as they may be a better fit for your use-case.

[Helius Webhooks](https://docs.helius.xyz/webhooks/webhooks-summary) - Helius is a service that allows you to create webhooks for your application. Helius will send a webhook to your application when a transaction is confirmed. This is a great option if you are looking for a managed solution with low overhead.


## Next Steps

In the tutorial, we will use websockets to broadcast transaction status updates to our web client in realtime. Check it out [here](/solana-pay/real-time-updates).
